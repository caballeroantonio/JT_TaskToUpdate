...
class ContenthistoryHelper
{
	...
	public static function prepareData(JTableContenthistory $table)
	{
		...
		$object = static::formatValues($object, $typesTable);
	...
	public static function processLookupFields($object, JTableContenttype $typesTable)
	{
         ...
         * xml formFile needs format like "%Y-%m-%d %H:%M" but
         * JHtml::date needs format like "Y-m-d H:i:s"
         * 
         * layout's format created, modified, checked, publish dates
         */
	public static function formatValues($object, JTableContenttype $typesTable)
	{
		...
		if ($formFile = static::getFormFile($typesTable))
		{
			if ($xml = simplexml_load_file($formFile))
			{
				$fieldArray = $xml->xpath('//field[@type="calendar" and @format!=""]');
				foreach ($fieldArray as $field)
				{
                                    $name = (string) $field->attributes()->name;
                                    $format = (string) $field->attributes()->format;
                                    $timezone = (string) $field->attributes()->filter;
                                    if(!$object->$name || in_array($name, $dateProperties))
                                        continue;
                                    $object->$name = JHtml::date($object->$name, strpos($format, ':')?'Y-m-d H:i':'Y-m-d', $timezone=='user_utc');
				}
			}
		}
		return $object;
